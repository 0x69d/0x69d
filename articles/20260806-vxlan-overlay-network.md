...
**オーバーレイ型**は、元のフレームを別のパケットで包みます。識別子を解釈するのは包む側と解く側の両端だけで、間の機器は外側のヘッダしか見ません。中身を知る必要がないので、物理構成から独立できます。

VXLANは後者です。L2フレームをUDPパケットに包むため、間のネットワークから見ればただのIPパケットが流れているだけになります。識別子（VNI）は24ビットあり、約1677万個のネットワークを区別できます。

2つを図にすると、識別子を誰が解釈するかの違いがはっきりします。

**ホップバイホップ型（例: VLAN）**

```mermaid
graph LR
    A1["Host A"] -->|"タグ付きフレーム"| S1["Switch<br/>タグを解釈して転送"]
    S1 -->|"タグ付きフレーム"| S2["Switch<br/>タグを解釈して転送"]
    S2 -->|"タグ付きフレーム"| A2["Host B"]
```

経路上のSwitchがすべて識別子を解釈する側に回っています。

**オーバーレイ型（VXLAN）**

```mermaid
graph LR
    B1["Host A<br/>VTEP"] -->|"外側ヘッダのみ<br/>（中身は見えない）"| R1["Router<br/>中身を知らずに転送"]
    R1 -->|"外側ヘッダのみ"| R2["Router<br/>中身を知らずに転送"]
    R2 -->|"外側ヘッダのみ"| B2["Host B<br/>VTEP"]
```

識別子を解釈するのは両端のVTEPだけで、途中のRouterは外側のIP/UDPヘッダしか見ていません。
...
図の上半分がオーバーレイ、下半分がアンダーレイで、`vxlan100`がその境目に立ちます。
...
VXLANインターフェースに直接IPを振ることもできますが、ブリッジを挟むとオーバーレイを「L2セグメント」として扱えます。このブリッジにvethやVMのtapを追加すれば、そのままオーバーレイに参加するインターフェースを増やせます。今回はブリッジ自身にIPを振って、VM本体をオーバーレイの住人にします。
...
- 今回の構成は、アンダーレイがマルチキャストを通せることを前提にしている
