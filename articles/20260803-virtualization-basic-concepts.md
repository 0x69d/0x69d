...
- **デバイス構成の柔軟性**: エミュレータによって多様なデバイス構成を再現できる
...
**ハイパーバイザー**とは、VMを作成・実行するための基盤となるソフトウェア層のことです。これから出てくるコンポーネントのうち、KVMとQEMUが組み合わさってこの役割を果たします。

その上で、実際に動くソフトウェアをカーネルに近い側から順に整理します。

- **KVM**: Linuxのカーネルモジュールです。CPUが持つハードウェア仮想化支援機能（Intel VT-x / AMD-V）を、`/dev/kvm`というインターフェースを通じてユーザースペースのソフトウェアに提供します。これによってLinuxカーネル自体がハイパーバイザーとして振る舞えるようになります。
- **QEMU**: ユーザースペースで動作するエミュレータです。VM 1台につき1つの`qemu-system-*`プロセスが起動し、ディスク・NIC・シリアルポートといった周辺デバイスをエミュレートします。ゲストのメモリ領域を確保するのもこのプロセスです。なお、KVMと組み合わせた場合、CPU命令はエミュレートされず物理CPU上で直接実行されます。QEMU単体でも動作しますが、その場合はCPUもソフトウェアでエミュレートするため非常に低速になります。
...
4. QEMUプロセスはKVMモジュールに対してハードウェア仮想化支援機能の利用を要求する
5. KVMはCPUのハードウェア仮想化支援機能を使い、QEMUが確保したメモリ領域の上でゲストのコードを直接実行させる
6. QEMUは仮想ディスク・仮想NICといった周辺デバイスをエミュレートし、仮想ハードウェアとしてゲストOSに提示する
